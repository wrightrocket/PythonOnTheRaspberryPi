...
if __name__ == '__main__':
    ...
    while True:
        if userChoice == 'q':
            ...
            '''
            lineText = '\t\t'
            for header in programChoices:
                lineText += '%8s\t' % (game.gameTypeDict[header])
            print lineText
            lineText = '%8s\t' % ('Player')
            for userCount in programChoices:
                lineText += '%8s\t' % (userTimes[userCount])
            print lineText
            lineText = '%8s\t' % ('Program')
            for programCount in programChoices:
                lineText += '%8s\t' % (programTimes[programCount])
            print lineText
            print
            ...
            print lineText
            for userCount in programChoices:
                lineText = '%8s\t' % (game.gameTypeDict[userCount])
                for programCount in programChoices:
                    lineText += '%8s\t' % (allTimes[userCount][programCount])
                print lineText
            if automatic:
                automaticTimes -= 1
                if automaticTimes == 0:
